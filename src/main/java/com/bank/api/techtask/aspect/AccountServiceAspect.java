...
public class AccountServiceAspect {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AccountServiceAspect.class);

    @Before("execution(* com.bank.api.techtask.service.AccountService.*(..))")
    ...
    @AfterReturning(pointcut = "execution(* com.bank.api.techtask.service.AccountService.*(..))", returning = "result")
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        logger.info("Exiting method: {} with result: {}", joinPoint.getSignature().toShortString(), result);
    }

    @AfterThrowing(pointcut = "execution(* com.bank.api.techtask.service.AccountService.*(..))", throwing = "error")
    public void logAfterThrowing(JoinPoint joinPoint, Throwable error) {
        logger.error("Exception in method: {} with cause: {}", joinPoint.getSignature().toShortString(), error.getMessage(), error);
    }

    @Around("execution(* com.bank.api.techtask.service.AccountService.*(..))")
